...
def prebuild(context, instance_uuid, flavor, image, availability_zone):
    ...
    payload = {
        ...
        "image": {
            ...
            # The Adapter approval catalog is pinned to Glance's SHA-512
            # os_hash_value.  Passing it across the authenticated Nova ->
            # Adapter boundary avoids a recursive Nova/Glance catalog lookup
            # while the original server-create request is still in flight.
            "checksum": (
                image.get("os_hash_value") or image.get("checksum") or
...
def _request(method, url, payload):
    ...
    try:
        ...
        raise exception.InvalidRequest(
            reason=(f"FLYT Adapter request failed with HTTP {error.code}: "
                    f"{detail}")
        ) from error
